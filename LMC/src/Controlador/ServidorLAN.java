...
public class ServidorLAN{
    ...
    /**
     * Método para configurar SSL antes de llamar a start().
     * @param rutaAlmacen Ruta del archivo JKS (keystore) que contiene el certificado del servidor.
     * @param contrasena Contraseña del keystore y de la clave privada.
     * @throws RuntimeException si ocurre algún error al cargar el keystore o inicializar SSL.
     */
    public void configurarSSL(String rutaAlmacen, String contrasena) {
        try {
            // Cargar el keystore
            KeyStore keyStore = KeyStore.getInstance("PKCS12");
            try (InputStream ksStream = new FileInputStream(rutaAlmacen)) {
                keyStore.load(ksStream, contrasena.toCharArray());
            }
            
            // Inicializar KeyManagerFactory
            KeyManagerFactory kmf = KeyManagerFactory.getInstance(KeyManagerFactory.getDefaultAlgorithm());
            kmf.init(keyStore, contrasena.toCharArray());
            
            // Crear SSLContext
            SSLContext sslContext = SSLContext.getInstance("TLS");
            sslContext.init(kmf.getKeyManagers(), null, null);
            
            // Obtener fábrica de SSLServerSocket
    ...
    /**
     * Comienza la conexión SSL en el puerto designado
     * @param PORT puerto donde se conecta (por defecto es el 8000)
     * @throws IllegalStateException se lanza la excepción si este método se usa antes de configurar SSL.
     */
    public void start(int PORT){
        // Verificar que SSL esté configurado (opcional: si no se configuró, lanzar excepción)
        if (!sslConfigurado) {
            throw new IllegalStateException("Se debe llamar a configurarSSL() antes de iniciar el servidor con SSL");
        }
        try{
            server= (SSLServerSocket)sslFactory.createServerSocket(PORT);
            inicio= true;
            new Thread(()-> {
                try{
                    while(true){
    ...
    public void actualizarTodos(){
        //<editor-fold defaultstate="collapsed" desc="actualiza los datos de todas las computadoras almacenados en la BD">
        gestionEquipo controladorCompu= new gestionEquipo();
        List<String> exclusiones= OpcionesConfig.LocalConfig.getExclusiones();
        enviarDatoATodos("equipo");
        ...
        for(Map<String, List<Object>> compu : computadoras){
            ...
            if(controladorCompu.estaRegistrada(ID) && !exclusiones.contains(ip)){
                ...
                compu.get("Sistema Operativo").set(3, 
                        fixCompu.get("Sistema Operativo").getLast()); //id de la BD del SO
                compu.get("CPU").set(2, fixCompu.get("CPU").getLast()); //id de la BD del CPU
                ...
                compu.get("RAM").set(3, fixCompu.get("RAM").getLast()); //id de la BD de la RAM
                ...
                compu.get("Motherboard").set(3, 
                        fixCompu.get("Motherboard").getLast()); //id de la BD de la Motherboard
                ...
                if(CD_DVD != null){
                   ...
                   if(fixCompu.get("Lector CD/DVD") != null){
                       if(noSerie_CD_DVD.equals("No disponible"))
                        ...
                        compu.get("Lector CD/DVD").set(3, 
                                fixCompu.get("Lector CD/DVD").getLast()); //id de la BD del Lector CD/DVD
                   }
                   else
                       compu.get("Lector CD/DVD").set(3, null); //id en null para el registro
                }
                /*si la marca del o los monitores y los discos duros obtenidos
                mediante OSHI es "No disponible"
                ...
                /*asigna a "compu" todos los datos incluido el id de todas las apps iguales de la BD
                (esto garantiza que se mantengan los atributos modificados en el sistema de las apps y
                que se excluyan apps desinstaladas, también se mantiene el id en null de las apps nuevas instaladas)
                y todos los cambios de "fixCompu"*/
                Iterator<Map.Entry<String, List<Object>>> it= fixCompu.entrySet().iterator();
                List<List<Object>> appsVerificadas= new LinkedList<>();
                while(it.hasNext()){
                    ...
                    if(entrada.getKey().contains("app")){
                        for(Map.Entry<String, List<Object>> entrada2 : compu.entrySet()){
                            if(entrada2.getKey().contains("app") && entrada2.getValue().getFirst().toString().
                                ...
                                if(appsVerificadas.contains(entrada2.getValue())) continue;
                                ...
                                appsVerificadas.add(entrada2.getValue());
                                it.remove();
                                break;
                            }
                        }
                ...
                /*se cuenta cant de apps de compu, esto es necesario para 
                ...
                /*operación: se asigna a compu las apps extras (apps agregadas en el sistema manualmente)
                ...
                //agrega a "compu" las sesiones de "fixCompu" si no son iguales manteniendo las ya obtenidas en "compu"
                    ...
                    if(clave.contains("login")){
                        ...
                        for(Map.Entry<String, List<Object>> entrada : compu.entrySet()){
                            if(entrada.getKey().contains("login") && entrada.getValue().get(0).equals(valor.get(0)) && 
                                ...
                                entrada.getValue().set(4, valor.get(4)); //solo asigna el id de la BD
                                existe= true;
                                break;
                            }
                        }
            ...
            else if(!controladorCompu.estaRegistrada(ID) && ID > 0){
